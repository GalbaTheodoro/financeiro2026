...
CONFIGURACOES_PADRAO: dict[str, tuple[str, str, bool]] = {
    ...
    "usuarios_incluidos": ("5", "Usuários já inclusos no valor da assinatura", True),
...
def configuracoes(db: Session, apenas_publicas: bool = False) -> dict:
    ...
    dados = {c.chave: (c.valor or "") for c in query.all()}
...
def limite_usuarios(db: Session, assinatura: Assinatura | None) -> int:
    """Quantos usuários a conta pode ter (inclusos + pacotes confirmados)."""
